#define SFML_STATIC
//ultralight is not building statically
#define ULTRALIGHT_STATIC_BUILD
...
int MainVast(Window& win, ss::SaveState& save, Player& player) {
	...
	while (win.IsOpen())
	{
		...
		while (accumulator >= dt)
		{
			...
			if (freeCamera) { //window shift does not speed up/slow down as player zooms in or out
				...
				auto colliders = TileMap::GenetateColliderPoints(map.GetCloseColliders(player), map);
				player.Move(grounded, collided, colliders, map);

				if(grounded)
					collided = false;

				nextPos = player.getGlobalBounds(); //estimate for the next position
				nextPos.left += player.velocity.x; //step forward in the x and the y
				nextPos.top += player.velocity.y;

				win.SetCenter(player.getPosition() + sf::Vector2f(player.getSize().x / 2.f, player.getSize().y / 2.f));
			}
			if (Keyboard::isKeyPressed(Keyboard::LControl) && (Keyboard::isKeyPressed(Keyboard::W) || Keyboard::isKeyPressed(Keyboard::Q))) { //these can be hard binded just cuz... yk?
				win.Close();
			}
...
#include "save.json"
	...
	Window win("Vast", sf::VideoMode::getDesktopMode().width, sf::VideoMode::getDesktopMode().height, save.raw["game settings"]["fullscreen"].get<bool>(), save.raw["game settings"]["vsync"].get<bool>(), save.raw["game settings"]["show fps"].get<bool>(), sf::Color(20, 20, 20));
	Player player(sf::Vector2f(50.f, 50.f), save);
	switch (homepage::Run(win, save, player)) {
	case homepage::HomePageAction::Play: {
		MainVast(win, save, player);
		break;
	}
	case homepage::HomePageAction::Exit:
		exit(0);
	default:
		exit(1);
	}
