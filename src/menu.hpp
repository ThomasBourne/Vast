...
#include "save.json"
                    ...
                    if (lastClicked->getString().toAnsiString().find("True") == std::string::npos && lastClicked->getString().toAnsiString().find("False") == std::string::npos) {
                        if ((char)win.GetEvnt().key.code + 'A' >= 'A' && (char)win.GetEvnt().key.code + 'A' <= 'Z') {
                        ...
                        else if ((char)win.GetEvnt().key.code >= 26 && (char)win.GetEvnt().key.code <= 35) {
                            std::cout << (char)((win.GetEvnt().key.code - 26) + '0') << std::endl;
                ...
                else if (win.GetEvnt().type == WinEvnt::KeyReleased && win.GetEvnt().key.code == KEY_BIND(p, Menu)) {
                    goto LOOP_END;
                }
			}
			win.Clear();
